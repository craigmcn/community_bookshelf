...
resources :api_tokens, only: [:index, :new, :create, :destroy]  ApiTokensController  (signed-in only, scoped to current_user)
...
### JSON API
- `/api/v1/books`, `/api/v1/readings` (full CRUD) are backed by `Api::V1::BaseController`, which does **not** inherit `ApplicationController` — it skips Clearance/session auth entirely in favor of a bearer token (`Authorization: Bearer <token>`) resolved against `ApiToken` (see below). `current_user` on this controller tree is the token-resolved user, not Clearance's session-based one — same method name deliberately, so the existing `BookPolicy`/`ReadingPolicy`/`policy_scope` classes work unmodified for both the HTML and API controllers with zero policy-layer changes. `Api::V1::BaseController` forces `request.format = :json` on every request regardless of `Accept` header, since this controller tree only ever renders jbuilder JSON.
- Every `/api/v1/*` action requires a valid token, including `Book`'s nominally-public `index?`/`show?` — there is no anonymous API access, even though the HTML `BooksController` allows it. Pundit failures render `{"error": "..."}` with 403 (not the HTML flow's flash+redirect); a missing/invalid token renders the same shape with 401; a token valid but missing the required scope also renders 403, with a distinct `"Token missing required scope: ..."` message.
...
- `ApiToken` (`user_id`, `name`, `token_digest`, `token_prefix`, `scopes` — a Postgres string array validated against the closed whitelist `ApiToken::SCOPES` (`read:books`/`write:books`/`read:readings`/`write:readings`) — `expires_at`, `last_used_at`) replaced the old single `User#api_token` column (`has_secure_token`, plaintext, unscoped, non-expiring) — a user can hold multiple named, independently-revocable tokens instead of one all-or-nothing credential. A token's plaintext value is `ApiToken::TOKEN_PREFIX` + `SecureRandom.hex(24)`; only its bcrypt digest (`bcrypt` was already a dependency via Clearance) is persisted, alongside a `token_prefix` (the literal prefix plus 8 hex chars) used both for display (`cb_a1b2c3d4…`) and to narrow `ApiToken.authenticate`'s DB lookup by an indexed column before the deliberately-slow bcrypt comparison runs. `ApiToken.generate!` is the only way to obtain the plaintext — it's set on a non-persisted `plaintext_token` accessor on the returned instance, shown to the user exactly once (`flash[:new_token]` on `ApiTokensController#create`, mirroring the old `flash[:api_token]` one-time-reveal pattern) and never stored or retrievable again.
- `Api::V1::BaseController#require_scope!(scope)` gates each action on `@current_api_token.scopes.include?(scope)`, called from a `before_action` in `Api::V1::BooksController`/`Api::V1::ReadingsController` (`read:*` for `index`/`show`, `write:*` for `create`/`update`/`destroy`/`bulk`) — this runs *before* `set_book`/`set_reading` and is a separate, coarser check from Pundit's `authorize`/`policy_scope` calls ("can this token even attempt this?" vs. "can this user do this to this record?"); both must pass.
- `ApiTokensController` (`/api_tokens`, index/new/create/destroy) is scoped to `current_user.api_tokens` like `ReadingChallengesController` is to `current_user.reading_challenges` — a non-owner token id 404s before `authorize` is reached. `ApiTokenPolicy` pins `record.user == user`, same pattern as `ReadingChallengePolicy`. Revoking a token (`destroy`) is a hard delete — there's no `revoked_at`/soft-delete column, since at this scale "revoked" and "gone" are the same thing. `User has_many :api_tokens, dependent: :destroy` covers cleanup on account deletion with no extra code in `delete_account!`.
- Rate limiting via `rack-attack` (`config/initializers/rack_attack.rb`) throttles `/api/*` per-token (120 req/min), not per-IP, since every successfully-authenticated request already carries a token — plus a secondary per-IP throttle (30 req/min) on requests with no valid token, guarding the token check itself from brute-forcing. `Rack::Attack.verified_api_token` resolves the bearer value's prefix against `ApiToken.active.exists?(token_prefix:)` before either throttle keys on it (keying on the prefix, not the full secret, since a throttle check has no need to run the slow bcrypt comparison) — bucketing on the raw header instead (caught in Copilot PR review on #110) would let an attacker dodge both throttles by sending a different garbage token on every request: each string gets its own untouched "api/token" bucket, and the header is never blank, so "api/ip-unauthenticated" never engages either. No exemption for moderator/admin tokens — a runaway script is equally disruptive regardless of the token owner's role. `rack-attack`'s Railtie auto-registers its middleware; no explicit `config.middleware.use` needed.
...
- `test_helper.rb`'s `auth_headers(user, scopes: ApiToken::SCOPES)` (default: every scope) generates a fresh `ApiToken` per call via `ApiToken.generate!` and returns the `Authorization` header — tests that need to verify scope enforcement pass a narrower `scopes:` list.
