...
- `/users/:id` is a member's public profile: avatar, bio, favorite genres, finished/currently-reading counts, and their public reviews. Any signed-in member can view any profile — the app has no anonymous browsing, so "public" means visible to the community, not the internet. Names throughout the app (Community Readings, "Added by") link here.
